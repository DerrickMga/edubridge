    ...
    <div class="max-w-2xl mx-auto" x-data="checkout()">
        ...
        <form method="POST" action="{{ route('payments.initiate', $course) }}" id="checkoutForm">
            ...
            <div class="card p-5 mb-5">
                ...
                <p class="text-xs text-slate-400 mb-4">Platform-wide flat rates &mdash; all subjects, all levels.</p>
                    ...
                    <label class="cursor-pointer">
                        ...
                        <div class="border-2 rounded-xl p-4 transition-all"
                            ...
                            <div class="flex justify-between items-start mb-2">
                                ...
                                @if($p['key'] === 'hourly')
                                    <span class="text-[10px] font-bold bg-sky-100 text-sky-700 px-1.5 py-0.5 rounded-full">QUICK</span>
                                @elseif($p['key'] === 'monthly')
                                    <span class="text-[10px] font-bold bg-violet-100 text-violet-700 px-1.5 py-0.5 rounded-full">FLEXIBLE</span>
                                @elseif($p['key'] === 'termly')
            ...
            <div class="card p-5 mb-5">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    @foreach([
                        ['stripe',    'bg-indigo-600', 'S',  'Stripe',    'Card (USD)',         'Visa / Mastercard / Amex'],
                        ['payfast',   'bg-blue-700',   'PF', 'PayFast',   'Card or EFT (ZAR)',  'South African banks'],
                        ['paynow_zw', 'bg-yellow-500', 'PN', 'Paynow',    'Bank (ZWG)',         'Zimbabwe banks'],
                        ['ecocash',   'bg-red-500',    'EC', 'EcoCash',   'Mobile (ZWG)',       'Econet wallet'],
                        ['innbucks',  'bg-green-600',  'IB', 'InnBucks',  'Mobile (USD)',       'Steward Bank'],
                    ] as [$val, $badge, $mono, $label, $desc, $sub])
                    <label class="cursor-pointer">
                        ...
                        <div class="border-2 rounded-xl p-3 transition-all"
                            ...
                            <div class="w-9 h-9 {{ $badge }} rounded-lg flex items-center justify-center mb-2">
                                <span class="text-white text-xs font-extrabold tracking-tight">{{ $mono }}</span>
                            </div>
            ...
            <div class="card p-5 mb-5 bg-slate-50">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-slate-800" x-text="periodLabel"></span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-500">Payment via</span>
                        <span class="font-medium text-slate-800" x-text="providerLabel"></span>
                    </div>
